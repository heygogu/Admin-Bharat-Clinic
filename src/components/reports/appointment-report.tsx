...
export function AppointmentReport() {
  ...
  useEffect(() => {
    const fetchAppointmentData = async () => {
      ...
      try {
        ...
        const statusResponse = await fetch(`/api/reports/appointments/status?period=${period}`)
        ...
        const dayResponse = await fetch(`/api/reports/appointments/day?period=${period}`)
        ...
        const timeResponse = await fetch(`/api/reports/appointments/time?period=${period}`)
  ...
  const DAY_COLORS = ['#8884d8', '#83a6ed', '#8dd1e1', '#82ca9d', '#a4de6c', '#d0ed57', '#ffc658']
